...
class GetEngine(object):
    ...
    def get_html(self):
        ...
        # In addition to finding HTML files we also scrub internal links (anchors), as they can't be translated to pdf (yet...).
        page_suffixes = []
        for file in self.files:
            if file.endswith(".xhtml") or file.endswith(".html"):
                self.html_files.append(file)
                baseName = os.path.basename(file)
                page_suffixes.append(baseName)

        for elem in self.html_files:
            fileContent = None
            with open(elem, 'rb') as f:
                fileContent = f.read()

            for suf in page_suffixes:
                fileContent = fileContent.replace(suf.encode(), b'')
